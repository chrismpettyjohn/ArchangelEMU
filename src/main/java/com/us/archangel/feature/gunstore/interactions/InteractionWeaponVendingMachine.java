...
import com.us.archangel.feature.gunstore.packets.outgoing.WeaponVendingMachineDataComposer;
...
public class InteractionWeaponVendingMachine extends InteractionDefault {
    ...
    public InteractionWeaponVendingMachine(int id, HabboInfo ownerInfo, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public void onClick(final GameClient client, final Room room, Object[] objects) throws Exception {
        if (!client.getHabbo().getPlayer().isWorking()) {
            client.getHabbo().whisper(Emulator.getTexts().getValue("roleplay.weapon_vending_machine.not_allowed"));
            return;
        }
        ...
        client.sendResponse(new WeaponVendingMachineDataComposer());
    }
}